...
Texture2D::Texture2D() 
		: m_width(0), m_height(0), channels(0)
{
...
void Texture2D::Generate(const char* fileName, int width, int height)
{
	fileExists(fileName);
...
bool Texture2D::fileExists(const char* fileName)
{
	FILE* file = fopen(fileName, "r");
	if (!file) {
		R3D_WARN("TEXTURE2D: Texture file does not exist");
		return false;
	}
	fclose(file);
	return true;
